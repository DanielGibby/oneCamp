  ...
  Drupal.behaviors.myBehavior = {
    attach: function (context, settings) {
      once('myBehavior', 'body', context).forEach(function (element) {
        ...
        $('.field--name-field-waitlist').find('.field--item').each(function() {
            $(this).text($(this).text().replace(/\./g,' '));
        });

        var first_name = $('#block-entityviewuser').find('.field--name-field-first-name').text();
        var last_name = $('#block-entityviewuser').find('.field--name-field-last-name').text();
        var member_number = $('#block-entityviewuser').find('.field--name-field-membership-number').text();

        var nid = $('.register-event').attr('data-nid');
        var button_id = $('.register-event').attr('id');
        var user = $('.register-event').attr('data-current-user');

        $('.register-event').attr('data-user-first-name', first_name);
        $('.register-event').attr('data-user-last-name', last_name);
        $('.register-event').attr('data-user-member-number', member_number);

        $('.register-event').on('click', function (e) {
          console.log($(this).attr('data-status'));
          e.preventDefault();
          if ($(this).attr('data-status') == 'not-registered') {
            $(this).attr('data-status', 'processing');
            $(this).addClass('processing');
            $(this).find('.svg-inline--fa').removeClass('fa-calendar-alt').removeClass('fa').addClass('fa-spinner').addClass('fas');
            $(this).find('.text-content').text('Processing your request.... Please wait.');

            $.ajax({
              type: 'POST',
              url: '/handle-ajax-calls',
              dataType: 'json',
              data: {
                function_call: "add-to-waitlist",
              ...
              success: function (data) {
                console.log(data);
                $('#' + data.button_id).attr('data-status', 'wait-list');
                $('#' + data.button_id).addClass('wait-list').removeClass('processing');
                $('#' + data.button_id).find('.svg-inline--fa').removeClass('fa-calendar-alt').addClass('fa-check-circle');
                $('#' + data.button_id).find('.text-content').text('You\'re on the waitlist!');
              },
              error: function (data) {
